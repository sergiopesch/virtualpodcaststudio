...
export default function VideoStudio() {
  ...
  const _handleTrackNameChange = (track: number, name: string) => {
    setProject(prev => ({
      ...
      trackSettings: {
        ...prev.trackSettings,
        [track]: { ...prev.trackSettings[track], name }
      }
    }));
  };
  ...
  const _handleTrackVolumeChange = (track: number, volume: number) => {
    setProject(prev => ({
      ...
      trackSettings: {
        ...prev.trackSettings,
        [track]: { ...prev.trackSettings[track], volume: Math.max(0, Math.min(1, volume)) }
      }
    }));
  };
  ...
  const _toggleTrackMute = (track: number) => {
    setProject(prev => ({
      ...
      trackSettings: {
        ...prev.trackSettings,
        [track]: { ...prev.trackSettings[track], mute: !prev.trackSettings[track].mute }
      }
    }));
  };
  ...
  const _handleRemoveTrack = (track: number) => {
    setProject(prev => {
      ...
      const newSettings = { ...prev.trackSettings };
      delete newSettings[track];
      return {
        ...prev,
        trackSettings: newSettings
      };
    });
  };
